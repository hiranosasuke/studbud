...
router.post(
  ...
  (req, res) => {
    ...
    groupFields.users = [];
    groupFields.users.push(req.body.owner);
...
router.delete(
  ...
  (req, res) => {
    ...
    User.findById(req.user.id).then(user => {
      user.groups.splice(user.groups.indexOf(req.user.id), 1);
      user.save();
    });
  }
);
...
router.post("/addUser", (req, res) => {
  Profile.findOne({ handle: req.body.handle })
    .then(profile => {
      Group.findById(req.body.group_id).then(group => {
        if (group.users.includes(profile.user)) {
          ...
          group.save();
          User.findById(profile.user._id).then(user => {
            user.groups.unshift(group._id);
            user.save();
          });
        }
    ...
    .catch(profile => res.status(404).json(group));
});
    ...
    groups.forEach(group => {
      group.users.forEach(user => {
        if (!users.hasOwnProperty(user)) {
          users[user] = [group._id];
        } else {
          users[user].push(group._id);
        }
      });
    });
    res.json({ users, total: total });
  });
});

router.post("/getJoinedGroups", (req, res) => {
...
router.post("/getFirstGroup", (req, res) => {
  User.findById(req.body.id)
    .populate("groups")
    .then(user => {
      res.json(user.groups);
